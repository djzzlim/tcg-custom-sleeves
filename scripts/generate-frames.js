...
const fadeSvg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${width} ${height}" width="${width}" height="${height}">
  ...
  <rect x="${thickness}" y="${thickness}" width="${width - 2*thickness}" height="${height - 2*thickness}" fill="none" stroke="white" stroke-width="40" filter="url(#blur)" opacity="1" />
...
// 7. Scallop (Outward semi-circles)
fs.writeFileSync(path.join(dir, '07.svg'), makePath(generateOuterAndInner(() => {
  let d = `M${thickness},${thickness}`;
  for(let x = thickness; x < width - thickness; x+=20) d += ` A10,10 0 0,1 ${x+20},${thickness}`;
  for(let y = thickness; y < height - thickness; y+=20) d += ` A10,10 0 0,1 ${width-thickness},${y+20}`;
  for(let x = width - thickness; x > thickness; x-=20) d += ` A10,10 0 0,1 ${x-20},${height-thickness}`;
  for(let y = height - thickness; y > thickness; y-=20) d += ` A10,10 0 0,1 ${thickness},${y-20}`;
  return d + ` Z`;
}), 'scallop'));

// 8. Stamp (Inward semi-circles)
fs.writeFileSync(path.join(dir, '08.svg'), makePath(generateOuterAndInner(() => {
  let d = `M${thickness},${thickness}`;
  for(let x = thickness; x < width - thickness; x+=20) d += ` A10,10 0 0,0 ${x+20},${thickness}`;
  for(let y = thickness; y < height - thickness; y+=20) d += ` A10,10 0 0,0 ${width-thickness},${y+20}`;
  for(let x = width - thickness; x > thickness; x-=20) d += ` A10,10 0 0,0 ${x-20},${height-thickness}`;
  for(let y = height - thickness; y > thickness; y-=20) d += ` A10,10 0 0,0 ${thickness},${y-20}`;
  return d + ` Z`;
}), 'stamp'));
...
fs.writeFileSync(path.join(dir, '09.svg'), makePath(generateOuterAndInner(() => {
  let d = `M${thickness},${thickness}`;
  for(let x = thickness; x < width - thickness; x+=40) d += ` Q${x+10},${thickness+10} ${x+20},${thickness} T${x+40},${thickness}`;
  for(let y = thickness; y < height - thickness; y+=40) d += ` Q${width-thickness-10},${y+10} ${width-thickness},${y+20} T${width-thickness},${y+40}`;
  for(let x = width - thickness; x > thickness; x-=40) d += ` Q${x-10},${height-thickness-10} ${x-20},${height-thickness} T${x-40},${height-thickness}`;
  for(let y = height - thickness; y > thickness; y-=40) d += ` Q${thickness+10},${y-10} ${thickness},${y-20} T${thickness},${y-40}`;
  return d + ` Z`;
}), 'wavy'));
